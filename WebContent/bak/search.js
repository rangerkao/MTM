angular.module('MainApp')
	.controller('searchCtrl',['Menu','AjaxService','CommonService','ActionService','$filter',function(Menu,AjaxService,CommonService,ActionService,$filter){
		...
		self.header = [
			...
			{name:'Last Visited Network',value:'lastVisitedNetwork'}
			];
		self.datas = [];
		self.tableDatas = [];
		/*for(var i = 0;i<56;i++){
		...
		self.queryDatas = function(){
			...
			.success(function(data, status, headers, config) {
				...
				if(data['error']){
					...
					//self.datas = mergeSort(data['data'],0,data['data'].length,'serviceid',false);
		...
		function flashSelectedStatus(){
			...
			angular.forEach(self.selectedItem, function(value, key) {
				...
				if(self.selectedStatus.indexOf(value.status)==-1){
					self.selectedStatus.push(value.status);
			...
			if(self.selectedStatus.indexOf('Normal')!=-1){
				angular.forEach(self.action, function(value, key) {
					//key 是 key or index
					if(value.label=='Resume'){
						self.action.splice(key, 1);
					}
				});
			}
			//如果有Suspend，禁止勾選Suspend
			if(self.selectedStatus.indexOf('Suspend')!=-1){
				angular.forEach(self.action, function(value, key) {
					//key 是 key or index
					if(value.label=='Suspend'){
						self.action.splice(key, 1);
					}
				});
			}
			//如果有Terminated，禁止勾選任何動作
			if(self.selectedStatus.indexOf('Terminated')!=-1){
				self.action = [];
			}
